...
// import { Play, Pause, X, ChevronRight, AlertTriangle, CheckCircle2, Clock } from "lucide-react";

// type ExecutionState = "idle" | "running" | "paused" | "completed" | "blocked";
// type StepStatus = "pending" | "running" | "allowed" | "blocked";
...
// export default function RuntimeScreen({ viewMode, compact = false, theme = "light" }: Props) {
//   const [executionState, setExecutionState] = useState<ExecutionState>("blocked");
//   const [userInput, setUserInput] = useState("");

//   const steps = [
//     { id: 1, name: "query_payroll_db", status: "allowed" as StepStatus, ms: 120 },
//     { id: 2, name: "summarize_results", status: "allowed" as StepStatus, ms: 340 },
//     { id: 3, name: "send_email", status: "blocked" as StepStatus, ms: 45 },
//   ];
...
//       hover: "hover:border-[#111827]",
//       bgSecondary: "bg-[#F9FAFB]",
//       bgTertiary: "bg-[#FAFAFA]",
//       borderDivider: "border-[#D1D5DB]",
//       actionName: "text-[#111827]",
//       actionBlockedBg: "bg-white",
//       actionBlockedText: "text-[#92400E]",
//     },
//     dark: {
//       bg: "bg-[#1E293B]",
...
//       borderDivider: "border-[#475569]",
//       actionName: "text-[#111827]",
//       actionBlockedBg: "bg-[#334155]",
//       actionBlockedText: "text-[#FDB022]",
//     },
//     "high-contrast": {
//       bg: "bg-white",
//       border: "border-black",
...
//       bgSecondary: "bg-[#F9FAFB]",
//       bgTertiary: "bg-[#FAFAFA]",
//       borderDivider: "border-[#9CA3AF]",
//       actionName: "text-black",
//       actionBlockedBg: "bg-white",
//       actionBlockedText: "text-[#92400E]",
//     },
//   };
...
//   const showTiming = viewMode === "engineering";
//   const showRisk = viewMode === "security";
...
//             placeholder="Enter agent request..."
//             className={`flex-1 px-3 py-2 border ${colors.border} ${colors.bgTertiary} ${colors.text} placeholder:${colors.textTertiary} focus:outline-none ${colors.hover} ${compact ? "text-xs" : "text-sm"}`}
//           />
//           <button className="px-4 py-2 bg-[#111827] text-white font-medium hover:bg-[#1F2937] transition-colors flex items-center gap-2">
//             <Play className="w-4 h-4" />
//             Run
//           </button>
//           <button className={`px-3 py-2 border ${colors.border} ${colors.textSecondary} ${colors.hover} transition-colors`}>
...
//             <span className="w-5 h-5 bg-[#111827] text-white flex items-center justify-center text-xs font-bold">1</span>
//             <span className={`text-xs font-semibold ${colors.textSecondary} uppercase tracking-wide`}>Request</span>
//           </div>
//           <p className={`text-sm ${colors.text} font-medium`}>Summarize payroll changes and email to regional managers</p>
//         </div>

//         {/* Plan */}
//         <div className={`${colors.bg} border ${colors.border} p-3`}>
//           <div className="flex items-center gap-2 mb-2">
//             <span className="w-5 h-5 bg-[#111827] text-white flex items-center justify-center text-xs font-bold">2</span>
//             <span className={`text-xs font-semibold ${colors.textSecondary} uppercase tracking-wide`}>Plan</span>
//           </div>
//           <div className="flex items-center gap-2 overflow-x-auto">
//             {steps.map((step, idx) => (
//               <div key={step.id} className="flex items-center gap-2">
//                 <div className={`flex items-center gap-2 px-3 py-1.5 ${colors.bgSecondary} border ${colors.border} whitespace-nowrap`}>
//                   <span className={`text-xs ${colors.textSecondary}`}>{step.id}</span>
//                   <span className={`text-sm ${colors.text}`}>{step.name}</span>
//                   {showTiming && <span className={`text-xs ${colors.textTertiary}`}>{step.ms}ms</span>}
//                 </div>
//                 {idx < steps.length - 1 && <ChevronRight className={`w-4 h-4 ${colors.textTertiary} flex-shrink-0`} />}
//               </div>
//             ))}
//           </div>
//         </div>
...
//               <span className="w-5 h-5 bg-[#111827] text-white flex items-center justify-center text-xs font-bold">3</span>
//               <span className={`text-xs font-semibold ${colors.textSecondary} uppercase tracking-wide`}>Evaluation</span>
//             </div>
//             <span className="px-2 py-0.5 bg-[#FEF3C7] border border-[#F59E0B] text-[#92400E] text-xs font-semibold">POL-017</span>
//           </div>
...
//                     <span className={`text-xs font-semibold ${colors.textSecondary}`}>{step.id}</span>
//                     <span className={`text-sm ${isBlocked || isAllowed ? colors.actionName : colors.text} font-medium`}>{step.name}</span>
//                   </div>
//                   <div className="flex items-center gap-2">
//                     {isAllowed ? (
//                       <>
//                         <span className="px-2 py-0.5 bg-[#D1FAE5] text-[#065F46] text-xs font-semibold">ALLOW</span>
//                         <CheckCircle2 className="w-4 h-4 text-[#10B981]" />
//                       </>
//                     ) : (
//                       <>
//                         <span className="px-2 py-0.5 bg-[#FEF3C7] text-[#92400E] text-xs font-semibold">BLOCKED</span>
...
//         {/* Decision */}
//         <div className={`${colors.actionBlockedBg} border-2 border-[#F59E0B] p-3`}>
//           <div className="flex items-center gap-2 mb-3">
//             <AlertTriangle className="w-5 h-5 text-[#F59E0B]" />
//             <span className={`text-sm font-bold ${colors.actionBlockedText}`}>Action Blocked</span>
//           </div>

//           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//             <div className="space-y-2">
//               <div>
//                 <span className={`text-xs font-semibold ${colors.textSecondary} uppercase tracking-wide block mb-1`}>Policy</span>
//                 <span className={`text-sm ${colors.text}`}>POL-017: External Data Transmission</span>
//               </div>
//               {showRisk && (
//                 <div>
//                   <span className={`text-xs font-semibold ${colors.textSecondary} uppercase tracking-wide block mb-1`}>Risk</span>
//                   <div className="flex items-center gap-2">
//                     <div className="flex-1 h-1.5 bg-[#F3F4F6]">
//                       <div className="h-full w-[85%] bg-[#EF4444]"></div>
//                     </div>
//                     <span className="text-xs font-bold text-[#EF4444]">8.5</span>
//                   </div>
//                 </div>
//               )}
//               <div>
//                 <span className={`text-xs font-semibold ${colors.textSecondary} uppercase tracking-wide block mb-1`}>Reason</span>
//                 <span className={`text-sm ${colors.text}`}>Confidential data cannot be sent externally</span>
//               </div>
//             </div>

//             <div>
//               <span className={`text-xs font-semibold ${colors.textSecondary} uppercase tracking-wide block mb-2`}>Actions</span>
//               <div className="space-y-2">
//                 <button className={`w-full px-3 py-2 border ${colors.border} text-left ${colors.hover} transition-colors text-sm ${colors.text}`}>
//                   Restrict to @company.com
//                 </button>
//                 <button className={`w-full px-3 py-2 border ${colors.border} text-left ${colors.hover} transition-colors text-sm ${colors.text}`}>
//                   Request HR approval
//                 </button>
